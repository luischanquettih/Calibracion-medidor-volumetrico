...
def ejecutar(codigos, nro_decimales):
    #nro_decimales = int(input("Número de decimales para el certificado: "))
    # ¿Se realizó el ajuste?
    ajuste = extraer_datos("informacion","realizar_ajuste")
    t_ref_cal = "20 °C"
    valvula_drenaje = extraer_datos("informacion","valvula_drenaje")


    # 1era hoja
    titulo = "CERTIFICADO DE CALIBRACIÓN"
    nombre_laboratorio = "Laboratorio de Grandes Volúmenes y Flujo"
    nro_certificado = extraer_datos("informacion","nro_certificado")
    cotizacion = extraer_datos("informacion","cotizacion")
    solicitante = extraer_datos("informacion","solicitante")
    direccion = extraer_datos("informacion","direccion")

    nombre_instrumento = "MEDIDOR VOLUMÉTRICO METÁLICO"

    marca = extraer_datos("informacion","marca")
    modelo = extraer_datos("informacion","modelo")
    nro_serie = extraer_datos("informacion","nro_serie")
    procedencia = extraer_datos("informacion","procedencia")
    cod_id = extraer_datos("informacion","cod_id")
    color = extraer_datos("informacion","color").capitalize()
    valor_nominal = extraer_datos("informacion","valor_nominal")+" "+extraer_datos("informacion","valor_nominal_unidad")
    interv_ind = "± "+extraer_datos("informacion","interv_ind")
    clase = extraer_datos("informacion", "clase")
    res = extraer_datos("informacion","res")+" "+extraer_datos("informacion","res_unidad")
    tipo = extraer_datos("informacion","tipo_SCM")
    material = extraer_datos("informacion", "material_SCM")
    c_material = coef_material[material]

    nombre_fecha_lugar = "FECHA Y LUGAR DE CALIBRACIÓN"
    fecha_cal = extraer_datos("informacion","fecha_cal")
    fecha_emi = extraer_datos("informacion","fecha_emi")
    ...
    if ajuste == "SI":
        vn = extraer_datos("resultados_antes_ajuste","VN_SCM")
        vn_unidad = extraer_datos("resultados_antes_ajuste","VN_SCM_unidad")
        vo_aa = extraer_datos("resultados_antes_ajuste","Vt")
        vo_aa_unidad = extraer_datos("resultados_antes_ajuste","Vt_unidad")
        U_aa = extraer_datos("resultados_antes_ajuste","U_Vt")
        U_aa_unidad = extraer_datos("resultados_antes_ajuste","U_Vt_unidad")

        vo_da = extraer_datos("resultados","Vt")
        vo_da_unidad = extraer_datos("resultados","Vt_unidad")
        U_da = extraer_datos("resultados","U_Vt")
        U_da_unidad = extraer_datos("resultados","U_Vt_unidad")
        #U_porcentaje_da = 0.04
        #U_porcentaje_da_unidad = "L" 
    else:
        vn = extraer_datos("resultados","VN_SCM")
        vn_unidad = extraer_datos("resultados","VN_SCM_unidad")
        vo_aa = extraer_datos("resultados","Vt")
        vo_aa_unidad = extraer_datos("resultados","Vt_unidad")
        U_aa = extraer_datos("resultados","U_Vt")
        U_aa_unidad = extraer_datos("resultados","U_Vt_unidad")
    ...
    t_inicial = extraer_datos("informacion","t_inicial").replace(".",",")+" "+extraer_datos("informacion","t_unidad")
    t_final = extraer_datos("informacion","t_final").replace(".",",")+" "+extraer_datos("informacion","t_unidad")
    h_inicial = extraer_datos("informacion","h_inicial").replace(".",",")+" "+extraer_datos("informacion","h_unidad")
    h_final = extraer_datos("informacion","h_final").replace(".",",")+" "+extraer_datos("informacion","h_unidad")
    p_inicial = extraer_datos("informacion","p_inicial").replace(".",",")+" "+extraer_datos("informacion","p_unidad")
    p_final = extraer_datos("informacion","p_final").replace(".",",")+" "+extraer_datos("informacion","p_unidad")


    dev_st = strRoundFloatPC(extraer_datos("resultados","desv_estandar"),nro_decimales)+" "+extraer_datos("resultados","desv_estandar_unidad")
    nro_repeticiones = extraer_datos("resultados","nro_repeticiones")
    u_deltaVmen = strRoundFloatPC(extraer_datos("resultados","u_deltaVmen"),nro_decimales)+" "+extraer_datos("resultados","u_deltaVmen_unidad")
    resolucion_calculada = strRoundFloatPC(extraer_datos("resultados","resolucion"),nro_decimales)+" "+extraer_datos("resultados","resolucion_unidad")+\
        " ("+strRoundFloatPC(extraer_datos("resultados","resolucion_porcentaje"),nro_decimales)+" "+extraer_datos("resultados","resolucion_porcentaje_unidad")+")"
    ...
    if valvula_drenaje == "SI":
        lista_obs.append(f"Tiempo de goteo del instrumento: {extraer_datos("informacion","tiempo_goteo")} s.")
        lista_obs.append(f"Tiempo de entrega del instrumento: {extraer_datos("informacion","tiempo_entrega")} s.")
    ...
    ruta_certificado = f"certificados/{nro_certificado}"
